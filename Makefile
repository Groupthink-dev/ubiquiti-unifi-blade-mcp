...
help:
	...
	@echo "  make test-e2e         - Run e2e tests (requires live UniFi controller)"
...
test-e2e:
	UNIFI_E2E=1 uv run pytest tests/e2e/ -v -m e2e

test-cov:
	uv run pytest tests/ -m "not e2e" --cov=src/ubiquiti_unifi_blade_mcp --cov-report=term-missing -v
...
type-check:
	uv run mypy src/ubiquiti_unifi_blade_mcp
...
run:
	uv run ubiquiti-unifi-blade-mcp
